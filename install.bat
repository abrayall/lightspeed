...
if exist "go.mod" (
    findstr /c:"lightspeed" go.mod >nul 2>&1
    if !errorlevel! equ 0 (
        if exist "build.bat" (
            goto :build_local
        )
...
set "TMP_DIR=%TEMP%\lightspeed-install-%RANDOM%"
mkdir "%TMP_DIR%" 2>nul
...
:: Copy binary
copy /y "%BINARY%" "%INSTALL_DIR%\lightspeed.exe" >nul
...
if %errorlevel% neq 0 (
    ...
    echo Add it to your PATH to run lightspeed from anywhere:
...
echo.
echo [92mSuccessfully installed lightspeed to %INSTALL_DIR%\lightspeed.exe[0m
echo.
