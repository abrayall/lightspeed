...
# Check if we're in the lightspeed git repo
is_in_repo() {
    if [ -f "go.mod" ] && grep -q "lightspeed" go.mod 2>/dev/null; then
        if [ -f "build.sh" ]; then
            return 0
        fi
    fi
...
install_binary() {
    ...
    if [ -w "$INSTALL_DIR" ]; then
        cp "$BINARY" "$INSTALL_DIR/lightspeed"
        chmod +x "$INSTALL_DIR/lightspeed"
    else
        sudo cp "$BINARY" "$INSTALL_DIR/lightspeed"
        sudo chmod +x "$INSTALL_DIR/lightspeed"
    fi
}
